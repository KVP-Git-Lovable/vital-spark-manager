-- Bulk-links or creates app patient records for a batch of Salesforce patients.
-- Input: jsonb array of {sf_id, name, phone} (phone = last 10 digits, may be '').
-- Returns counts so the importer can report progress.
...
BEGIN
  ...
  -- Drop the ones already present.
  DELETE FROM _sf_batch b USING public.patients p WHERE p.sf_id = b.sf_id;

  -- Link by phone when exactly one unlinked app patient carries that number.
  WITH cand AS (
    SELECT b.sf_id, MIN(p.id) AS patient_id, COUNT(*) AS n
    FROM _sf_batch b
