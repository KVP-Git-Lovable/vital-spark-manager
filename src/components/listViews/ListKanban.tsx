// Generic kanban board, extracted from
// src/components/patients/listviews/PatientKanban.tsx. Field config and
// row-value resolution are passed in, and the card's leading visual
// (Patients shows a PatientAvatar) is pluggable via renderLeading.
...
interface Props {
  ...
  /** Field shown as the card title. Defaults to the first column. */
  titleField?: string;
  /** Optional leading visual on each card (e.g. avatar). */
  renderLeading?: (row: any) => React.ReactNode;
}
...
export default function ListKanban({
  rows, config, options, columns, fields, rawValue = defaultRawValue, onOpen, onMove, titleField, renderLeading,
}: Props) {
  ...
  }, [rows, options, config.group_field, rawValue]);

  const summaryLabel = config.summarize_field ? fieldDefIn(fields, config.summarize_field)?.label : null;

  const secondary = columns.filter((c) => c !== title && c !== config.group_field).slice(0, 3);
                ...
                {g.rows.map((row) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      {renderLeading?.(row)}
                      <p className="truncate text-sm font-medium text-primary">{formatCell(row, title)}</p>
                    </div>
                    <div className="mt-2 space-y-0.5">
                      {secondary.map((key) => (
                        <p key={key} className="truncate text-xs text-muted-foreground">
                          {fieldDefIn(fields, key)?.label}: {formatCell(row, key)}
                        </p>
                      ))}
                    </div>
                  </div>
