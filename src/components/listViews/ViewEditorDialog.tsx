// Generic saved-view editor, extracted from
// src/components/patients/listviews/ViewEditorDialog.tsx. Field config and
// picklist-option resolution are passed in (instead of importing
...
import { type FieldDef, OPERATORS, fieldDefIn, type FilterCondition, type ListView } from "@/lib/listViews/engine";
...
interface Props {
  ...
  optionsFor: (source?: string) => PickOption[];
  people: PickOption[];
  /** Plural, lowercase noun for the "no filters" hint, e.g. "patients", "appointments". Defaults to "records". */
  itemLabel?: string;
}
...
function FieldSelect({ fields, value, onChange }: { fields: FieldDef[]; value: string; onChange: (key: string) => void }) {
...
export function FieldPicker({ fields, columns, onChange }: { fields: FieldDef[]; columns: string[]; onChange: (next: string[]) => void }) {
...
export default function ViewEditorDialog({
  open, onOpenChange, view, onSave, fields, defaultColumns, optionsFor, people, itemLabel = "records",
}: Props) {
        ...
        <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain">
          <div className="p-6 space-y-7 bg-card">
            ...
            <div className="space-y-2">
              ...
              {conditions.length === 0 && (
                <p className="text-sm text-muted-foreground border border-dashed border-border rounded-lg bg-muted/20 p-4">
                  No filters — this view shows all {itemLabel}.
                </p>
              )}
            ...
            <div className="space-y-2">
              ...
              <FieldPicker fields={fields} columns={columns} onChange={setColumns} />

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <div className="space-y-1">
                  <Label className="text-sm font-medium">Sort by</Label>
                  <FieldSelect fields={fields} value={sortField} onChange={setSortField} />
                </div>
                <div className="space-y-1">
